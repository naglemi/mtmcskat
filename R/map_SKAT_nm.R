map_SKAT_nm <- function(null_model_index,
  ...
  # The above function returns None if there are no SNPs within the window.
  #   Because of this, we only run the below if a matrix is returned to avoid
  #   "Z is not a matrix" error.
  for(i in 1:length(pos_and_SNP_list)){

    if(is.matrix(pos_and_SNP_list[[i]][[2]])==TRUE){

      this_SKAT_out <- SKAT_one_window(
        this_position = pos_and_SNP_list[[i]][[1]],
