...
#'
#' @param Z Matrix with row for each genotype in the SNP dataset,
#'   columns for each SNP in the SNP window of interest,
#'   and numeric values of 0, 1, 2 indicating number of alternative alleles
#'   for each SNP in each genotype (or NA for indels or missing data)
#' @param n_permutations Integer indicating the number of permutations used
#'   to calculate empirical p-values
#' @param null_model Object generated by \code{\link[SKAT]{SKAT_Null_Model}}
#' @param return_all_p If `TRUE`, will return a vector of all permuted p-values
#'   (useful for when not all permutations can fit into memory available for
#'   a single thread); if `FALSE`, will calculate and return the empirical
#'   p-value
#' @param ... Additional parameters passed on to \code{\link[SKAT]{SKAT}}
#'
#' @return An empirical p-value or vector of permuted p-values, depending on
#'   the user-submitted argument for `return_all_p` as described above
#' @export
#'
#' @examples
