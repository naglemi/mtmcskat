...
grab_window <- iter(function() extract_window(this_position = i,
...
calculate_SKAT_empirical_p <- function(n_permutations, null_model){
  this_SKAT_out <- SKAT(Z, null_model, kernel = "linear.weighted")
  p_resampled_SKAT <- (length(subset(this_SKAT_out$p.value.resampling,
                                     this_SKAT_out$p.value.resampling <= this_SKAT_out$p.value)))/n_permutations
}

runSKAToneChr <- function(raw_file_path, window_size, window_shift, output_dir = "/scratch2/NSF_GWAS/Results/SKAT/", ncore=24, SKAT_O = "OFF"){
    ...
    cl <- makeCluster(ncore, type = "FORK", outfile="cluster.outfile")
    registerDoParallel(cl)
    on.exit(stopCluster(cl))
    
    print("Begin parallel action")
    master_output <- foreach(i=window_list, .combine="rbind", .export=c("window_size")) %dopar% {
...
# Run tests ---------------------------------------------------------------
SKAT_one_window <- function(this_position, window_size, Z, SKAT_O = "OFF", raw_file_path){
